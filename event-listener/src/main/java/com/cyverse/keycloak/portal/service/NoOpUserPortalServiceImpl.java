package com.cyverse.keycloak.portal.service;

import org.jboss.logging.Logger;
import org.keycloak.models.UserModel;

public class NoOpUserPortalServiceImpl implements UserPortalService {
    private static final Logger logger = Logger.getLogger(NoOpUserPortalServiceImpl.class);

    @Override
    public void addUserToPortal(UserModel user) {
        logger.error("No operation possible. Check other dependencies for failures.");
    }
}
